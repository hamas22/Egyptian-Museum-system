...
if (!$conn->connect_error) {
  if ($is_admin) {
    $result = $conn->query("SELECT user_Id, type_of_membership FROM membership");
    while ($row = $result->fetch_assoc()) {
      $memberships[] = $row;
    }
  } else if ($user_Id) {
    $stmt = $conn->prepare("SELECT type_of_membership FROM membership WHERE user_Id = ?");
          ...
          <div class="meta-bottom d-flex justify-content-between align-items-center">
            ...
            if ($is_admin) {
              foreach ($memberships as $mem) {
                if ($mem['type_of_membership'] === $type) {
                  echo '<form method="post" action="admin_delete_membership.php" style="display:inline-block;">
                    <input type="hidden" name="user_Id" value="' . $mem['user_Id'] . '">
                    <input type="hidden" name="type" value="' . $type . '">
                  <button type="submit" class="btn" style="background-color:red; color:white; padding: 6px 8px; margin-bottom: 8px;">Delete</button>
                  </form>';
                }
              }
            } else {
